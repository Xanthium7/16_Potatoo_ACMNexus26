...
class CleanRoomResponse {
  ...
  send(body) {
    ...
    // Only default to text/plain if the route handler didn't specify a Content-Type natively
    if (!this.nativeRes.getHeader('Content-Type')) {
  ...
  listen(port, callback) {
    ...
    this.server = http.createServer((req, res) => {
      ...
      const matchedRoute = this.routes.find(route =>
        route.method === req.method && route.path === reqPath
      );
