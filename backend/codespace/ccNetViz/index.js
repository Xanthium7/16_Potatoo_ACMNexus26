    ...
    draw() {
        ...
        for (const node of this._nodes) {
            ...
            if (node.label && nodeStyle.label && nodeStyle.label.hideSize < nodeSize) {
                ...
                ctx.font = `${fontStyle.weight} ${fontStyle.size / scaleX}px ${fontStyle.family}`; // Adjust font size for zoom
                ctx.fillStyle = Color.fromRGBString(labelStyle.color).toRGBAString();
                ctx.textAlign = fontStyle.alignment;
                ctx.textBaseline = 'middle';

                const textWidth = ctx.measureText(node.label).width;
                const textHeight = fontStyle.size / scaleX;

                // Label background
                ctx.fillStyle = Color.fromRGBString(labelStyle.backgroundColor).toRGBAString();
                ctx.fillRect(node.x - textWidth / 2 - 5, node.y + nodeSize / 2 + 5, textWidth + 10, textHeight + 5);

                // Label border
                ctx.strokeStyle = Color.fromRGBString(labelStyle.borderColor).toRGBAString();
                ctx.strokeRect(node.x - textWidth / 2 - 5, node.y + nodeSize / 2 + 5, textWidth + 10, textHeight + 5);

                // Label text
                ctx.fillStyle = Color.fromRGBString(labelStyle.color).toRGBAString();
                ctx.fillText(node.label, node.x, node.y + nodeSize / 2 + 5 + textHeight / 2);
            }
        }
